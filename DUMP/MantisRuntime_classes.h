...
struct UFortGameplayAbility_Mantis : UFortGameplayAbility {
	...
	void OnMontageFinished(); // Function MantisRuntime.FortGameplayAbility_Mantis.OnMontageFinished // (Final|Native|Protected) // @ game+0x658e4e4
	void OnMontageCancelled(); // Function MantisRuntime.FortGameplayAbility_Mantis.OnMontageCancelled // (Final|Native|Protected) // @ game+0x658e490
	void ConfigureRootMotionWarping(struct AActor*& OutTargetActor, bool& bOutAllowTranslationWarp, bool& bOutAllowRotationWarp, bool& bOutSnapshotTargetLocation); // Function MantisRuntime.FortGameplayAbility_Mantis.ConfigureRootMotionWarping // (Native|Event|Protected|HasOutParms|BlueprintEvent) // @ game+0x658e18c
	void BP_OnMantisTechniqueHit(struct FGameplayAbilityTargetDataHandle& TargetDataHandle, struct FGameplayTag ApplicationTag); // Function MantisRuntime.FortGameplayAbility_Mantis.BP_OnMantisTechniqueHit // (Event|Protected|HasOutParms|BlueprintEvent) // @ game+0xda05cc
	void BP_MantisAbilityTechniqueStarted(struct FGameplayTagContainer OwningTags, struct FName SequenceName, bool StartsSequence, bool EndsSequence); // Function MantisRuntime.FortGameplayAbility_Mantis.BP_MantisAbilityTechniqueStarted // (Event|Protected|BlueprintEvent) // @ game+0xda05cc
};
...
struct UFortItemLayerAnimInstance_UncleBrolly : UFortItemLayerAnimInstance {
	struct FCachedAnimStateArray UmbrellaGliderOpenStateDataArray; // 0x458(0x18)
	struct FVector WrapTranslation; // 0x470(0x18)
	struct FRotator WrapRotation; // 0x488(0x18)
	struct FRotator RootRotationOffset; // 0x4a0(0x18)
	float LandingPredictedTimer; // 0x4b8(0x04)
	bool bShouldUpdateYawCorrection; // 0x4bc(0x01)
	bool bWasAcceleratingBeforeDodge; // 0x4bd(0x01)
	float UncleBrollyDeployCurveValue; // 0x4c0(0x04)
	float DashChargeTier; // 0x4c4(0x04)
	float DashChargeAdditivePlayrate; // 0x4c8(0x04)
	bool bIsUmbrellaFailing; // 0x4cc(0x01)
	bool bIsMeleeGuarding; // 0x4cd(0x01)
	bool bIsSprinting; // 0x4ce(0x01)
	bool bIsInAir; // 0x4cf(0x01)
	bool bIsLandingPredicted; // 0x4d0(0x01)
	bool bIsJumping; // 0x4d1(0x01)
	bool bIsDodging; // 0x4d2(0x01)
	bool bIsDodgingEast; // 0x4d3(0x01)
	bool bIsDodgingSouth; // 0x4d4(0x01)
	bool bIsDodgingWest; // 0x4d5(0x01)
	bool bIsDashing; // 0x4d6(0x01)
	bool bIsDashCharging; // 0x4d7(0x01)
	bool bIsDashPredictingEnd; // 0x4d8(0x01)
	bool bIsChargeTier1; // 0x4d9(0x01)
	bool bIsChargeTier2; // 0x4da(0x01)
	bool bIsChargeTier3; // 0x4db(0x01)
	bool bMeleeGuardingOrUmbrellaFailing; // 0x4dc(0x01)
	bool bTransition_Default_to_GuardLoop; // 0x4dd(0x01)
	bool bTransition_Default_to_GuardIntro; // 0x4de(0x01)
	bool bTransition_GuardIntro_to_Default; // 0x4df(0x01)
	bool bUpperBodyShouldPassThrough; // 0x4e0(0x01)
	bool bUmbrellaGliderIsOpen; // 0x4e1(0x01)
...
struct UFortMantisPawnComponent : UPawnComponent {
	...
	void SetLockOnState(bool bEnabled, struct AActor* TargetActor); // Function MantisRuntime.FortMantisPawnComponent.SetLockOnState // (Final|Native|Public|BlueprintCallable) // @ game+0x658e5d4
	void OnPostPhysicsRotation(struct UCharacterMovementComponent* CharMoveComp, float DeltaSeconds); // Function MantisRuntime.FortMantisPawnComponent.OnPostPhysicsRotation // (Final|Native|Protected) // @ game+0x658e50c
	void OnOwnerWeaponHolstered(); // Function MantisRuntime.FortMantisPawnComponent.OnOwnerWeaponHolstered // (Final|Native|Protected) // @ game+0x658e4f8
	void OnCharacterMovementPreUpdate(struct UCharacterMovementComponent* CharMoveComp, float DeltaSeconds); // Function MantisRuntime.FortMantisPawnComponent.OnCharacterMovementPreUpdate // (Final|Native|Protected) // @ game+0x658e3c8
	bool IsPerformingLockOn(); // Function MantisRuntime.FortMantisPawnComponent.IsPerformingLockOn // (Final|Native|Public|BlueprintCallable|BlueprintPure|Const) // @ game+0x658e3ac
	bool HasLockOnTarget(); // Function MantisRuntime.FortMantisPawnComponent.HasLockOnTarget // (Final|Native|Public|BlueprintCallable|BlueprintPure|Const) // @ game+0x658e330
	struct AActor* GetLockOnTarget(); // Function MantisRuntime.FortMantisPawnComponent.GetLockOnTarget // (Final|Native|Public|BlueprintCallable|BlueprintPure|Const) // @ game+0x658e30c
};
...
struct UFortMantisWeaponComponent : UFortWeaponComponent {
	...
	struct UFortMantisPawnComponent* MantisPawnComponent; // 0xa8(0x08)
